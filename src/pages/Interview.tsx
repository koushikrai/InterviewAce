...
//
// Components moved outside main Interview to avoid remounts on parent re-renders
//
...
type InterviewSetupProps = {
  ...
  setInterviewMode: (m: 'text' | 'voice') => void;
...
type ActiveInterviewProps = {
  ...
  isRecording: boolean;
  lastFeedback: { score: number; feedback: string } | null;
  lastFeedbackDetail: FeedbackResponse | null;
  setCurrentQuestion: (n: number) => void;
  setUserAnswerLocal: (s: string) => void; // alias to keep signature clear
};

export const ActiveInterview = ({
  currentQuestion, questions, interviewMode,
  userAnswer, setUserAnswer, textareaRef,
  handleAnswerSubmit, toggleRecording, isRecording,
  lastFeedback, lastFeedbackDetail, setCurrentQuestion, setUserAnswerLocal
    ...
    return (
      <Card className="max-w-4xl mx-auto">
        <CardContent className="pt-6 text-center">
          <p className="text-gray-600">No questions available. Please start a new interview.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const validQuestionIndex = Math.min(Math.max(0, currentQuestion), questions.length - 1);
    ...
    <Card>
      <CardContent className="pt-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-sm text-gray-600">Question {validQuestionIndex + 1} of {questions.length}</span>
        </div>
        <Progress value={(validQuestionIndex + 1) / questions.length * 100} />
      </CardContent>
    </Card>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl">Question {validQuestionIndex + 1}</CardTitle>
          <div className="flex gap-2">
            <Badge variant="secondary">{(currentQ as { type?: string })?.type}</Badge>
            <Badge variant="outline">{(currentQ as { expectedDuration?: string })?.expectedDuration ?? '2-3 minutes'}</Badge>
          </div>
        </div>
        {interviewMode === 'voice' && (
      ...
      <CardContent>
        <p className="text-lg leading-relaxed mb-6">{currentQ.question}</p>

        <div className="space-y-4">
          {interviewMode === 'text' ? (
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="mockinterview-answer">Your Answer</label>
              <Textarea
                id="mockinterview-answer"
                name="mockinterview-answer"
                ref={textareaRef}
                value={userAnswer}
          ...
          <div className="flex justify-between pt-4">
            <Button variant="outline" disabled={validQuestionIndex === 0} onClick={() => {
              if (validQuestionIndex > 0) {
                setCurrentQuestion(validQuestionIndex - 1);
                setUserAnswerLocal("");
              }
            }}>Previous Question</Button>
            <Button onClick={handleAnswerSubmit}>{validQuestionIndex < questions.length - 1 ? (<>Next Question <SkipForward className="w-4 h-4 ml-2" /></>) : ("Finish Interview")}</Button>
          </div>
        </div>
      </CardContent>
    </Card>
...
type InterviewFeedbackProps = {
  finalResults: FinalResultsType | null;
  reset: () => void;
...
export const InterviewFeedback = ({ finalResults, reset, sessionId, navigate, toast }: InterviewFeedbackProps) => (
  <div className="max-w-4xl mx-auto space-y-6">
    <Card>
      <CardHeader>
    ...
    <div className="flex gap-4 justify-center">
      <Button onClick={reset} variant="outline">New Interview</Button>
      <Button onClick={() => {
        if (sessionId) {
...
//
// Main Interview (holds state, keeps children stable)
//
const Interview = () => {
  ...
  // stable callbacks to avoid extra re-renders of children (not remount)
  ...
  }, [selectedRole, interviewMode, selectedDifficulty, selectedCount, toast]);
  ...
  const handleAnswerSubmit = useCallback(async () => {
    ...
    setCurrentQuestion((cq) => {
      ...
      if (nextQuestion < questions.length) {
        ...
        // Last question - move to feedback
        if (sessionId) {
          withApiFallback<AxiosResponse<FinalResultsType> | null>(
        ...
        return cq; // Keep current question index for now
      }
    });
  }, [userAnswer, sessionId, currentQuestion, questions, toast]);
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        {interviewStep === 'active' && (
          <ActiveInterview
            ...
            lastFeedbackDetail={lastFeedbackDetail}
            setCurrentQuestion={setCurrentQuestion}
            setUserAnswerLocal={setUserAnswer}
          />
        )}
